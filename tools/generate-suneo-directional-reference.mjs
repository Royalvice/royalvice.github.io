...
const RUN_ID = process.env.SUNEO_DIRECTIONAL_RUN || "2026-07-20-suneo-directional-reference-r1";
const ROUND = process.env.SUNEO_DIRECTIONAL_ROUND || "r1";
const FILTER = process.env.SUNEO_DIRECTIONAL_DIRECTION || null;
const COUNT = Math.max(1, Number(process.env.SUNEO_DIRECTIONAL_COUNT || 1));
...
const directions = [
  {
    ...
    seed: 17100,
    description:
      "小夫严格面向画面下方，是真正正面朝向观察者的中立站立姿势；保持五人中偏瘦小的儿童体型，横向夸张的黑色尖发 wedge 和两个明显尖端保持原始非对称方向，尖嘴、窄脸、绿色格纹上衣、白色领子、橙棕短裤、白袜和绿色鞋正面清楚可见，双肩和双脚稳定落在同一条水平基线。"
  },
  {
    id: "up",
    seed: 17101,
    description:
      "小夫严格背向观察者并面向画面上方；看到横向黑色尖发 wedge 的后侧和两个尖端、绿色格纹上衣背面、橙棕短裤背面、白袜和绿色鞋后部，保持瘦小儿童体型和原始尖发方向，不出现正面眼睛、鼻子或嘴；双脚落在同一条水平基线，双脚下方保持连续纯蓝背景空白，不绘制任何椭圆、彩色地面形状或投影。"
  },
  {
    id: "left",
    seed: 17102,
    description:
      "小夫严格朝画面左侧的正交 profile side view；窄长尖嘴、鼻子和嘴朝左，只显示一只眼睛，尖发 wedge 从头顶向身后右侧横向伸出并保留两个明显尖端，不能镜像尖发方向；绿色格纹上衣、白色领子、橙棕短裤、白袜和绿色鞋保持完整，身体和头部完全朝左，不是斜向正面。"
  }
];
...
function buildPrompt(direction) {
  return [
    ...
    "Input image: Image 1 is the only identity reference; preserve the exact Suneo identity, small slim child proportions, sideways black spiky hair wedge with two long points, narrow pointed face, green checkered shirt, white collar, orange-brown shorts, white socks, green shoes and pixel density.",
    "Subject: 小夫，五人中偏瘦小的儿童，横向夸张黑色尖发 wedge，尖嘴窄脸，绿色格纹上衣、白色领子、橙棕短裤、白袜和绿色鞋。",
    ...
    "Background: perfectly flat solid #0000FF chroma-key color, uniform and untextured.",
...
async function generate(direction, candidateIndex) {
  ...
  const outputPath = path.join(outputDir, `suneo-direction-${direction.id}-${suffix}.png`);
  ...
  form.append("images", new Blob([bytes], { type: "image/png" }), "suneo-front-reference.png");
  ...
  console.log(`submitted 小夫 ${direction.id} ${suffix} (${queued.job_id})`);
...
async function main() {
  ...
  console.log(`Saved ${results.length} Suneo directional references under ${path.relative(ROOT, ROOT_RUN)}`);
}
