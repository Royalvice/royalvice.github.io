...
OUT_GLB = ROOT / "public/assets/gallery/models/wooden-gallery-cabinet-v5.glb"
OUT_REPORT = ROOT / "public/assets/gallery/models/wooden-gallery-cabinet-v5.report.json"
OUT_LIGHTMAP = ROOT / "public/assets/gallery/materials/cabinet_lightmap_v5.png"
...
MATERIALS = {
    "mat_walnut_outer": (0.24, 0.125, 0.064, 1.0),
    "mat_warm_inner_wall": (0.74, 0.38, 0.18, 1.0),
    "mat_warm_side_wall": (0.86, 0.46, 0.22, 1.0),
    "mat_inner_floor": (0.31, 0.17, 0.085, 1.0),
    "mat_shadow_groove": (0.060, 0.022, 0.006, 0.34),
...
def write_lightmap_png() -> None:
    ...
    size = 1024
    image = bpy.data.images.new("cabinet_lightmap_v5", width=size, height=size, alpha=True, float_buffer=False)
    pixels: list[float] = []
    for y in range(size):
        v = y / (size - 1)
        for x in range(size):
            u = x / (size - 1)
            lamp = max(0.0, 1.0 - abs(v - 0.84) / 0.42)
            hero_bounce = max(0.0, 1.0 - ((u - 0.42) ** 2 + (v - 0.55) ** 2) ** 0.5 / 0.58)
            floor_falloff = max(0.0, 1.0 - v)
            corner = min(u, 1.0 - u, v, 1.0 - v)
            edge_shadow = 0.82 + min(0.18, corner * 1.7)
            warmth = 0.54 + lamp * 0.30 + hero_bounce * 0.20 + floor_falloff * 0.08
            r = min(1.0, warmth * 1.10 * edge_shadow)
            g = min(1.0, warmth * 0.86 * edge_shadow)
            b = min(1.0, warmth * 0.60 * edge_shadow)
...
def build_cabinet() -> dict:
    ...
    for index, (slot_x, slot_z) in enumerate(SLOT_CENTERS):
        ...
        # v5 visible wood-board seams. These are shallow physical grooves rather
        ...
        for groove_i, offset in enumerate((-0.30, 0.30)):
            ...
            panel(
                ...
                (left_x + PANEL + 0.012, 0.03, slot_z + offset),
                (0.010, DEPTH - 0.28, 0.012),
                "mat_shadow_groove",
                bevel=0.002,
                tile_scale=0.12,
        ...
        # v5 keeps only narrow physical corner darkening. v3 used wide matte AO
