...
OUT_GLB = ROOT / "public/assets/gallery/models/wooden-gallery-cabinet-v6.glb"
OUT_REPORT = ROOT / "public/assets/gallery/models/wooden-gallery-cabinet-v6.report.json"
OUT_LIGHTMAP = ROOT / "public/assets/gallery/materials/cabinet_lightmap_v6.png"
...
MATERIALS = {
    "mat_walnut_outer": (0.34, 0.20, 0.115, 1.0),
    "mat_warm_inner_wall": (0.62, 0.40, 0.245, 1.0),
    "mat_warm_side_wall": (0.66, 0.425, 0.260, 1.0),
    "mat_inner_floor": (0.50, 0.325, 0.205, 1.0),
    "mat_shadow_groove": (0.050, 0.018, 0.006, 0.22),
    "mat_black_lacquer_trim": (0.010, 0.009, 0.008, 1.0),
    "mat_brass_trim": (0.86, 0.57, 0.25, 1.0),
    "mat_light_diffuser": (1.0, 0.79, 0.46, 1.0),
    "mat_glass_edge": (0.70, 0.93, 1.0, 0.30),
    "mat_glass_pane": (0.82, 0.96, 1.0, 0.10),
    ...
    "mat_soft_shadow": (0.0, 0.0, 0.0, 0.22),
}
...
def create_material(name: str, color: tuple[float, float, float, float]) -> bpy.types.Material:
    ...
    if bsdf:
        ...
        if "Roughness" in bsdf.inputs:
            if "shadow_groove" in name:
                bsdf.inputs["Roughness"].default_value = 0.92
            elif "warm_inner_wall" in name or "warm_side_wall" in name:
                bsdf.inputs["Roughness"].default_value = 0.48
            elif "inner_floor" in name:
...
def write_lightmap_png() -> None:
    OUT_LIGHTMAP.parent.mkdir(parents=True, exist_ok=True)
    size = 1024
    image = bpy.data.images.new("cabinet_lightmap_v6", width=size, height=size, alpha=True, float_buffer=False)
    pixels: list[float] = []
    for y in range(size):
        v = y / (size - 1)
        for x in range(size):
            u = x / (size - 1)
            lamp = max(0.0, 1.0 - abs(v - 0.82) / 0.52)
            hero_bounce = max(0.0, 1.0 - ((u - 0.44) ** 2 + (v - 0.54) ** 2) ** 0.5 / 0.72)
            corner = min(u, 1.0 - u, v, 1.0 - v)
            edge_shadow = 0.84 + min(0.14, corner * 1.55)
            value = 0.88 + lamp * 0.10 + hero_bounce * 0.08
            value = max(0.76, min(1.0, value * edge_shadow))
            # v6b lightmap deliberately stays bright and warm. It should shape
            # cabinet depth, not convert walnut floors or side walls into slate.
            r = min(1.0, value * 1.055)
            g = min(1.0, value * 1.005)
            b = min(1.0, value * 0.900)
            pixels.extend((r, g, b, 1.0))
    image.pixels.foreach_set(pixels)
    image.filepath_raw = str(OUT_LIGHTMAP)
...
def build_cabinet() -> dict:
    ...
    for index, (slot_x, slot_z) in enumerate(SLOT_CENTERS):
        ...
        panel(f"{prefix}.back-panel", (slot_x, BACK_Y - PANEL / 2, slot_z), (INNER_W, PANEL, INNER_H), "mat_warm_inner_wall", bevel=0.014, tile_scale=1.45)
        panel(f"{prefix}.floor-panel", (slot_x, 0, bottom_z + PANEL / 2), (INNER_W, DEPTH, PANEL), "mat_inner_floor", bevel=0.014, tile_scale=1.25)
        panel(f"{prefix}.ceiling-panel", (slot_x, 0, top_z - PANEL / 2), (INNER_W, DEPTH, PANEL), "mat_warm_inner_wall", bevel=0.014, tile_scale=1.25)
        panel(f"{prefix}.left-panel", (left_x + PANEL / 2, 0, slot_z), (PANEL, DEPTH, INNER_H), "mat_warm_side_wall", bevel=0.014, tile_scale=1.2)
        panel(f"{prefix}.right-panel", (right_x - PANEL / 2, 0, slot_z), (PANEL, DEPTH, INNER_H), "mat_warm_side_wall", bevel=0.014, tile_scale=1.45)

        # v6 visible wood-board seams. These are shallow physical grooves rather
        # than dark overlays: they give the side/back walls readable furniture
        # scale while the warm wall material remains the dominant surface.
        for groove_i, offset in enumerate((-0.52, 0.0, 0.52)):
            panel(
                f"{prefix}.back-vertical-groove-{groove_i}",
                (slot_x + offset, BACK_Y - PANEL - 0.013, slot_z),
                (0.012, 0.010, INNER_H - 0.22),
                "mat_shadow_groove",
                bevel=0.002,
                tile_scale=0.12,
            )
        for groove_i, offset in enumerate((-0.34, 0.34)):
            panel(
                f"{prefix}.back-horizontal-groove-{groove_i}",
                (slot_x, BACK_Y - PANEL - 0.014, slot_z + offset),
                (INNER_W - 0.24, 0.010, 0.012),
                "mat_shadow_groove",
                bevel=0.002,
                tile_scale=0.12,
            )
        for groove_i, offset in enumerate((-0.30, 0.30)):
            panel(
                f"{prefix}.right-wall-horizontal-groove-{groove_i}",
                (right_x - PANEL - 0.012, 0.03, slot_z + offset),
                (0.010, DEPTH - 0.28, 0.012),
                "mat_shadow_groove",
                bevel=0.002,
                tile_scale=0.12,
            )

        for groove_i, y_offset in enumerate((-0.22, 0.18)):
            panel(
                f"{prefix}.floor-long-grain-groove-{groove_i}",
                (slot_x, y_offset, bottom_z + PANEL + 0.012),
                (INNER_W - 0.20, 0.010, 0.010),
                "mat_shadow_groove",
                bevel=0.0015,
                tile_scale=0.10,
            )
        for groove_i, x_offset in enumerate((-0.42, 0.38)):
            panel(
                f"{prefix}.floor-cross-board-groove-{groove_i}",
                (slot_x + x_offset, 0.02, bottom_z + PANEL + 0.014),
                (0.010, DEPTH - 0.30, 0.010),
                "mat_shadow_groove",
                bevel=0.0015,
                tile_scale=0.10,
            )
        for groove_i, wall_z_offset in enumerate((-0.30, 0.30)):
            panel(
                f"{prefix}.left-wall-horizontal-groove-{groove_i}",
                (left_x + PANEL + 0.012, 0.03, slot_z + wall_z_offset),
                (0.010, DEPTH - 0.28, 0.012),
                "mat_shadow_groove",
                bevel=0.002,
                tile_scale=0.12,
        ...
        # v6 keeps only narrow physical corner darkening. v3 used wide matte AO
        # strips and the cabinet collapsed into black; these are intentionally
        # slim enough to define contact without becoming the visible material.
        add_ao_strip(f"{prefix}.ao-back-floor", (slot_x, BACK_Y - PANEL - 0.012, bottom_z + 0.036), (INNER_W - 0.18, 0.010, 0.050), materials["mat_shadow_groove"], root)
        add_ao_strip(f"{prefix}.ao-left-floor", (left_x + PANEL + 0.012, 0.03, bottom_z + 0.034), (0.018, DEPTH - 0.22, 0.050), materials["mat_shadow_groove"], root)
        add_ao_strip(f"{prefix}.ao-right-floor", (right_x - PANEL - 0.012, 0.03, bottom_z + 0.034), (0.018, DEPTH - 0.22, 0.050), materials["mat_shadow_groove"], root)
        add_ao_strip(f"{prefix}.ao-back-right-corner", (right_x - PANEL - 0.012, BACK_Y - PANEL - 0.014, slot_z), (0.018, 0.010, INNER_H - 0.34), materials["mat_shadow_groove"], root)
        add_ao_strip(f"{prefix}.ao-back-left-corner", (left_x + PANEL + 0.012, BACK_Y - PANEL - 0.014, slot_z), (0.018, 0.010, INNER_H - 0.34), materials["mat_shadow_groove"], root)
        add_ao_strip(f"{prefix}.ao-ceiling-back", (slot_x, BACK_Y - PANEL - 0.012, top_z - PANEL - 0.046), (INNER_W - 0.26, 0.010, 0.040), materials["mat_shadow_groove"], root)

        panel(f"{prefix}.light-housing", (slot_x, FRONT_Y + 0.34, top_z - 0.105), (1.18, 0.34, 0.066), "mat_black_lacquer_trim", bevel=0.018, tile_scale=0.5)
        panel(f"{prefix}.light-diffuser", (slot_x, FRONT_Y + 0.34, top_z - 0.144), (0.98, 0.25, 0.018), "mat_light_diffuser", bevel=0.010, tile_scale=0.45)
        panel(f"{prefix}.light-brass-lip", (slot_x, FRONT_Y + 0.160, top_z - 0.106), (1.22, 0.032, 0.024), "mat_brass_trim", bevel=0.007, tile_scale=0.2)
        panel(f"{prefix}.light-baffle-back", (slot_x, FRONT_Y + 0.535, top_z - 0.132), (1.17, 0.028, 0.068), "mat_black_lacquer_trim", bevel=0.008, tile_scale=0.20)
        panel(f"{prefix}.light-baffle-right", (slot_x + 0.610, FRONT_Y + 0.34, top_z - 0.130), (0.030, 0.30, 0.064), "mat_black_lacquer_trim", bevel=0.006, tile_scale=0.16)
        ...
        panel(f"{prefix}.plaque-body", (plaque_x, glass_y - 0.030, plaque_z), (PLAQUE_W, 0.022, PLAQUE_H), "mat_black_lacquer_trim", bevel=0.006, tile_scale=0.16)
        panel(f"{prefix}.plaque-screen", (plaque_x, glass_y - 0.047, plaque_z), (PLAQUE_W - 0.052, 0.010, PLAQUE_H - 0.052), "mat_plaque_lcd", bevel=0.004, tile_scale=0.205)
        ...
        add_empty(f"{prefix}.light-anchor", (0, FRONT_Y + 0.34, INNER_H / 2 - 0.14), parent=slot)
    ...
    return {
        ...
        "lightmap": str(OUT_LIGHTMAP.relative_to(ROOT)),
        "slots": slots,
    }
...
def main() -> None:
    ...
    report["lightmapBytes"] = OUT_LIGHTMAP.stat().st_size
    report["passed"] = True
