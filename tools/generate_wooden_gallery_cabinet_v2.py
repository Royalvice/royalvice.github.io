...
OUT_GLB = ROOT / "public/assets/gallery/models/wooden-gallery-cabinet-v2.glb"
OUT_REPORT = ROOT / "public/assets/gallery/models/wooden-gallery-cabinet-v2.report.json"
...
def create_material(name: str, color: tuple[float, float, float, float]) -> bpy.types.Material:
    ...
    if bsdf:
        ...
        if "Roughness" in bsdf.inputs:
            if "lacquer" in name or "obsidian" in name:
...
def build_cabinet() -> dict:
    ...
    for index, (slot_x, slot_z) in enumerate(SLOT_CENTERS):
        ...
        panel(f"{prefix}.right-panel", (right_x - PANEL / 2, 0, slot_z), (PANEL, DEPTH, INNER_H), "mat_cherry_interior", bevel=0.014, tile_scale=1.2)
