...
const VARIANT_KEY = process.env.NOBITA_REVERSE_VARIANT || "r1";
const VARIANTS = {
  r1: {
    runId: "2026-07-22-nobita-left-reverse-from-a14-r1",
    prompt: "反转参考图中的走路姿势，左脚改到后面，右脚改到前面，身子保持朝左竖直。",
    seedBase: 80000
  },
  r2: {
    runId: "2026-07-22-nobita-left-reverse-from-a14-r2",
    prompt: "反转人物双腿的前后位置：左脚在后，右脚在前。",
    seedBase: 81000
  }
};
...
const REFERENCE = path.join(
  ROOT,
  "artifacts/profile-sprite-review/2026-07-22-nobita-left-two-frame-clean-r1/frame-a/candidate-14.png"
);
const REFERENCE_SHA256 = "97274ec46daaf94f412ebd8100bc22e8b5856b17464587d13082f80ff5399b83";
const PROMPT = VARIANT?.prompt;
const SEED_BASE = VARIANT?.seedBase;
const MAX_CANDIDATES = 16;
const BATCH_START = Number(process.env.NOBITA_REVERSE_BATCH_START || 1);
const BATCH_SIZE = Number(process.env.NOBITA_REVERSE_BATCH_SIZE || 4);
...
async function generate(candidateIndex, referenceBytes) {
  ...
  if ((await exists(outputPath)) || (await exists(promptPath)) || (await exists(jobPath))) {
    throw new Error(`Refusing to overwrite reverse candidate ${candidateIndex}.`);
  }
  ...
  console.log(`submitted reverse candidate ${candidateIndex} (${queued.job_id})`);
...
async function main() {
  ...
  if (!Object.hasOwn(VARIANTS, VARIANT_KEY)) {
    throw new Error("NOBITA_REVERSE_VARIANT must be exactly 'r1' or 'r2'.");
  }
  if (!Number.isInteger(BATCH_START) || BATCH_START < 1) {
    throw new Error("NOBITA_REVERSE_BATCH_START must be a positive integer.");
  }
  if (!Number.isInteger(BATCH_SIZE) || BATCH_SIZE < 1 || BATCH_SIZE > 4) {
    throw new Error("NOBITA_REVERSE_BATCH_SIZE must be an integer from 1 to 4.");
  }
  ...
  const requestRecord = {
    runId: RUN_ID,
    variant: VARIANT_KEY,
    method: "single-image-walk-pose-reversal",
    reference: relative(REFERENCE),
    referenceSha256: REFERENCE_SHA256,
    referenceSize,
    referencePose: "左脚在前、右脚在后",
    inputImageCount: 1,
    prompt: PROMPT,
    requestParams: REQUEST_PARAMS,
    seedBase: SEED_BASE,
    maxCandidates: MAX_CANDIDATES,
    inputPolicy: "每个候选只上传同一张已生成的朝左走路姿势参考图，不上传中立图、其他帧或其他资产。"
  };
  if (!(await exists(requestPath))) {
    ...
    if (JSON.stringify(existing) !== JSON.stringify(requestRecord)) {
      throw new Error(`Existing ${relative(requestPath)} does not match this reversal experiment.`);
    }
  }
  ...
  console.log(`Saved reverse candidates ${BATCH_START}-${batchEnd} under ${relative(CANDIDATES)}`);
}
